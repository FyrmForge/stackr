...
func tileFlags(c *cobra.Command) map[string]string {
	...
	for _, s := range [][2]string{
		{"image", "image ref (image tiles)"}, {"git", "git URL (service tiles)"}, {"branch", "git branch"},
		{"dockerfile", "Dockerfile path"}, {"context", "build context"}, {"watch-paths", "paths whose change triggers a build, one per line"},
		{"env-json", "environment, a JSON object"}, {"build-args", "build args, a JSON object"}, {"volumes", "volume mounts"},
		{"command", "the command"}, {"published-ports", "ports published on the host"}, {"protocol", "endpoint protocol"},
	...
	return map[string]string{
		"image": "image_ref", "git": "git_url", "branch": "git_branch", "dockerfile": "dockerfile_path", "context": "build_context",
		"watch-paths": "watch_paths", "env-json": "env_json", "build-args": "build_args", "volumes": "volumes", "command": "command",
		"published-ports": "published_ports", "protocol": "endpoint_protocol", "health-path": "health_path", "healthcheck": "healthcheck_cmd",
		"user": "user", "devices": "devices", "restart": "restart_policy", "depends-on": "depends_on", "files": "files",
...
func (a *app) stackCommands() []*cobra.Command {
	return []*cobra.Command{a.stacks(), a.releases(), a.envs(), a.promote(), a.rollback(), a.tiles(),
		a.deploy(), a.restart(), a.logs(), a.managed(), a.params(), a.volumes(), a.backups()}
}

// at runs f with the path down to lv (tile from the first arg, if any).
func (a *app) at(lv level, f func(c *cobra.Command, p string, args []string) error) func(*cobra.Command, []string) error {
...
func (a *app) stacks() *cobra.Command {
	var desc string
	create := leaf("create <name>", "stack.create", "Make a stack", exact(1), a.at(atOrg, func(_ *cobra.Command, p string, args []string) error {
	...
	configRepo := leaf("config-repo", "stack.config-repo", "Point the stack at its config repo", exact(0), a.at(atStack, func(_ *cobra.Command, p string, _ []string) error {
		_, err := a.call(PUT, p+"/config-repo", map[string]string{"repo": repo, "branch": branch, "path": path, "connector_id": conn})
		return err
	}))
	...
	var withEnv bool
	reserve := leaf("reserve", "stack.reservations", "Reserve the stack's domain", exact(0), a.at(atStack, func(_ *cobra.Command, p string, _ []string) error {
		_, err := a.call(PUT, p+"/reservations", map[string]any{"host": host, "acme_email": email, "include_env_on_default": withEnv})
		return err
	}))
	...
	return scoped(noun("stack", "Stacks",
		leaf("ls", "stack.list", "List stacks", exact(0), a.at(atOrg, func(_ *cobra.Command, p string, _ []string) error {
		...
		waits(leaf("image-check", "stack.image-check", "Check the stack's images for a newer tag", exact(0), a.at(atStack, func(c *cobra.Command, p string, _ []string) error {
			return a.orgJob(c, POST, p+"/image-check", nil, "image check")
		}))),
		leaf("rm", "stack.delete", "Delete the stack", exact(0), a.at(atStack, func(c *cobra.Command, p string, _ []string) error {
...
func (a *app) releases() *cobra.Command {
	return scoped(noun("release", "Releases of the stack",
		...
		leaf("get <number>", "release.list,release.get", "Show a release and its pins", exact(1), a.at(atStack, func(c *cobra.Command, p string, args []string) error {
...
func (a *app) envs() *cobra.Command {
	...
	var auto bool
	create := leaf("create <name>", "env.create", "Make an environment", exact(1), a.at(atStack, func(_ *cobra.Command, p string, args []string) error {
		body := map[string]any{"name": args[0], "type": typ, "color": color, "from_kind": fromKind, "from_branch": fromBranch, "auto": auto}
		if base != "" {
	...
	from := leaf("from", "env.from", "Set where the env's releases come from", exact(0), a.at(atEnv, func(_ *cobra.Command, p string, _ []string) error {
	...
	return scoped(noun("env", "Environments of the stack",
		...
		leaf("reorder <env>...", "env.list,env.reorder", "Set the promote order, bottom rung first", atLeast(1), a.at(atStack, func(_ *cobra.Command, p string, args []string) error {
		...
		leaf("rm", "env.delete", "Delete the environment", exact(0), a.at(atEnv, func(c *cobra.Command, p string, _ []string) error {
...
func (a *app) plan(p, rel string) (bool, error) {
	...
	for _, c := range changes {
		...
		rows = append(rows, []string{cell(ch["kind"]), cell(ch["tile"]), cell(ch["field"]), cell(ch["old"]), cell(ch["new"]), cell(ch["note"])})
	}
...
func (a *app) tiles() *cobra.Command {
	...
	var exCont string
	exec := leaf("exec [tile] -- <cmd>...", "tile.status,tile.exec", "Run a command in a replica; stdin goes to it", atLeast(1), nil)
	...
	return scoped(noun("tile", "Tiles: the deployed units",
		...
		create, set,
		leaf("rename <tile> <name>", "tile.rename", "Rename a tile", exact(2), a.at(atTile, func(_ *cobra.Command, p string, args []string) error {
...
func (a *app) domains() *cobra.Command {
	domainFlags := func(c *cobra.Command) map[string]string {
		...
		return map[string]string{"path": "path", "port": "port", "https": "https", "force-https": "force_https", "redirect-to": "redirect_to"}
	}
	...
	set := leaf("set <host>", "domain.list,domain.update", "Change a domain; only the flags given change", exact(1), nil)
	...
	set.RunE = a.at(atTile, func(c *cobra.Command, p string, args []string) error {
		...
		body := map[string]any{"host": cur["host"], "path": cur["path"], "port": cur["container_port"], "https": cur["https"],
			"force_https": cur["force_https"], "redirect_to": cur["redirect_to"]}
	...
	var file string
	caddy := leaf("caddy <host>", "domain.list,domain.raw-caddy", "Set a domain's raw Caddy config (admin); -f - reads stdin, empty clears", exact(1),
		a.at(atTile, func(_ *cobra.Command, p string, args []string) error {
			...
			_, err = a.call(PUT, p+"/domains/"+cell(cur["id"])+"/raw-caddy", map[string]string{"raw_caddy": string(raw)})
			return err
		}))
	...
	return noun("domain", "The tile's domains",
		...
		add, set, caddy,
		leaf("rm <host>", "domain.list,domain.detach", "Stop routing a host to the tile", exact(1), a.at(atTile, func(_ *cobra.Command, p string, args []string) error {
			cur, err := a.find(p+"/domains", "domain", args[0], "host", "id")
			if err != nil {
				return err
			}
...
func (a *app) slices() *cobra.Command {
	...
	var public bool
	attach := leaf("attach <instance-tile-id>", "slice.attach", "Give the tile a slice of a managed tile (a database, a bucket)", exact(1),
		a.at(atTile, func(c *cobra.Command, p string, args []string) error {
			return a.orgJob(c, POST, p+"/slices", map[string]any{"instance_tile_id": args[0], "name": name, "public": public, "on_remove": onRemove}, "attach")
		}))
	...
	return noun("slice", "Slices of managed tiles bound to this tile",
		a.get("ls [tile]", "slice.list", "List the tile's slices", atTile, "/slices", "slug", "db_name", "public", "on_remove", "id"),
		attach,
		leaf("detach <slice-id>", "slice.detach", "Unbind a slice; --on-remove decided what happens to its data", exact(1), a.at(atOrg, func(_ *cobra.Command, p string, args []string) error {
...
func (a *app) managed() *cobra.Command {
	...
	sc := leaf("scope <tile>", "managed.scope", "Who may take slices: env, stack or org", exact(1), a.at(atTile, func(_ *cobra.Command, p string, _ []string) error {
	...
	return scoped(noun("managed", "Managed tiles",
		a.get("ls", "managed.list", "List the env's managed tiles", atEnv, "/managed", "engine", "endpoint", "scope_kind", "tile_id", "id"),
		create, sc,
	), true)
}
...
func (a *app) params() *cobra.Command {
	...
	send := func(c *cobra.Command, lines []string) error {
		...
		for _, l := range lines {
			...
			body = append(body, map[string]string{"collection": col, "name": name, "kind": kind, "value": v})
		}
		if len(body) == 0 {
	...
	set := leaf("set <collection.NAME=value>...", "org.params-set,stack.params-set,env.params-set",
		"Set params; others at the level stay", atLeast(1), func(c *cobra.Command, args []string) error { return send(c, args) })
	merge := leaf("merge", "org.params-set,stack.params-set,env.params-set",
		"Set every collection.NAME=value line of a file (-f, \"-\" stdin); others stay", exact(0), func(c *cobra.Command, _ []string) error {
	...
	return levelFlag(noun("params", "The param store: org, stack and env levels",
		get, set, merge, export,
		leaf("rm <collection.NAME>", "org.param-delete,stack.param-delete,env.param-delete", "Delete a param at the level", exact(1),
...
func (a *app) volumes() *cobra.Command {
	var maxMB int
	add := leaf("add <slug>", "org.volume-declare,stack.volume-declare,env.volume-declare", "Declare a volume at the level", exact(1),
		func(c *cobra.Command, args []string) error {
			p, err := a.levelPath(c)
			if err != nil {
				return err
	...
	return levelFlag(noun("volume", "Volumes",
		leaf("ls", "org.volumes,stack.volumes,env.volumes", "List volumes at the level", exact(0), func(c *cobra.Command, _ []string) error {
			p, err := a.levelPath(c)
			if err != nil {
				return err
			}
		...
		add,
		leaf("rm <volume-id>", "volume.delete", "Delete a volume and its data", exact(1), a.at(atOrg, func(_ *cobra.Command, p string, args []string) error {
...
func (a *app) backups() *cobra.Command {
	...
	schedKeys := func(c *cobra.Command) map[string]string {
		...
		return map[string]string{"method": "method", "dest": "dest_id", "schedule": "cron", "tz": "timezone", "keep": "keep", "mode": "mode"}
	}
	...
	create.RunE = org(func(c *cobra.Command, p string, args []string) error {
		body := map[string]any{"method": flag(c, "method"), "cron": flag(c, "schedule"), "timezone": flag(c, "tz"), "mode": flag(c, "mode")}
	...
	set := leaf("set <volume-id> <schedule-id>", "schedule.list,schedule.update", "Change a backup schedule; only the flags given change", exact(2), nil)
	...
	set.RunE = org(func(c *cobra.Command, p string, args []string) error {
		...
		body := map[string]any{"method": cur["method"], "dest_id": cur["dest_id"], "cron": cur["cron"],
			"timezone": cur["timezone"], "keep": cur["keep"], "mode": cur["mode"]}
		for k, v := range ch {
			body[k] = v
		}
	...
	now := waits(leaf("run <volume-id>", "backup.now", "Back a volume up now", exact(1), org(func(c *cobra.Command, p string, args []string) error {
		return a.orgJob(c, POST, p+"/volumes/"+args[0]+"/backups", map[string]string{"dest_id": dest, "method": method, "mode": mode}, "backup")
	})))
	...
	restore := waits(leaf("restore <volume-id>", "backup.restore", "Restore a backup run over a volume", exact(1), org(func(c *cobra.Command, p string, args []string) error {
		if runID == "" {
		...
		return a.orgJob(c, POST, p+"/volumes/"+args[0]+"/restore", map[string]string{"run_id": runID, "target_volume_id": target}, "restore")
	})))
	...
	return noun("backup", "Volume backups",
		...
		create, set,
		leaf("rm <schedule-id>", "schedule.delete", "Delete a backup schedule", exact(1), org(func(_ *cobra.Command, p string, args []string) error {
