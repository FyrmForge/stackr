...
- [x] step-7.md org config file: `stackr-org.yml` bound to the org, plans as rows an owner approves or an Auto switch applies, apply = one job through the orchestrator's verbs, never deletes a stack; plus v0's new-org wizard one to one; planned 2026-09-25 (DECIDE 180 to 193). Done 2026-09-25 on `rewrite-step-7`, VM proof v0.0.36 (wizard by hand, Config tab, CLI; bind, push, plan, approve, ladder envs, webhook plan, auto apply, never-deletes, `moved:` rename, `domains:` + `apex:`, export round trip) green; the proof fixes are v0.0.37.
