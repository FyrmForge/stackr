...
| `stacks` | stack | id, org_id → orgs CASCADE, name, slug, description, settings, config_connector_id, config_repo, config_branch, config_path, created_at | `org_declared`, `ui_edits`, `proxy_middlewares` gone. `config_*` stay: the release pins the config repo's commit. `UNIQUE (org_id, slug)`. |
...
| `domains` | domain | id, tile_id → tiles CASCADE, host, path, container_port, https, force_https, redirect_to, auto, position, proxy_json, raw_caddy, created_at | `cert_pem`/`key_pem` gone (Caddy automatic TLS). `middlewares`/`priority`/`rule` (Traefik) → `proxy_json` named extras + admin-only `raw_caddy`. Unique index on `(host, path)`. |
