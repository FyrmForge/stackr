...
- Encrypted at rest (`enc1:` prefix, `service/internal/secrets`): param
  ...
  provision passwords, binding passwords and outputs, destination secret key
  and archive key.
...
| `tiles` | tile | id, stack_id → stacks CASCADE, environment_id → environments CASCADE, name, slug, kind `service\|image\|managed\|cron\|function\|slice`, git_url, git_branch, image_ref, dockerfile_path, build_context, watch_paths, env_json, build_args, volumes (mount lines), command, container_port, published_ports, endpoint_protocol, health_path, healthcheck_cmd, healthcheck_interval_s, healthcheck_timeout_s, healthcheck_retries, healthcheck_start_period_s, cpu_limit, mem_limit_mb, user, shm_size_mb, privileged, devices, restart_policy, depends_on, files, shared_net, replicas, update_policy `manual\|auto`, tag_policy, schedule, trigger, paused, timeout_minutes, provision_from (nullable), default_access `read\|write` (nullable), slice_access (JSON list, default `[]`), created_at, updated_at | `status` gone (derived). `source_type` folded into `kind`. `env` → `env_json` (literal-or-ref block). Cron columns (`cron`, `allow_overlap`, `timeout_minutes`, `last_*`, `run_on_deploy`) gone; cron and function tiles carry `schedule`, `trigger`, `paused`, `timeout_minutes` instead. A slice tile (DECIDE 194) carries `provision_from` (`<stack>:<env>:<tile>`, stored as written, refs and all) and `default_access`; both are NULL on every other kind. `slice_access` is a consumer's `[{from, access}]`: `from` a slice tile slug in the same env, `access` `read\|write`; a slice the consumer refs with no entry gets the slice's `default_access`. `basic_auth_*`, `sec_headers`, `traefik_override` → `domains.proxy_json`. `connector_id` gone (resolved from `git_url` host). `webhook_token` gone (pushes arrive through the connector). `external_port` gone (`published_ports`). `endpoint_port_var`, `storage`, `wait_for_ci`, `home_node`, `node_group` gone. `image_digest`/`latest_digest` → `images` + `release_tiles.digest`. New: `health_path`, `tag_policy`. `UNIQUE (environment_id, slug)`. |
...
| `volumes` | volume | id, scope_kind `env\|stack\|org`, scope_id, instance_id → managed_instances SET NULL (nullable), slug, name (Docker volume), max_size_mb, orphaned_at (nullable), created_at | new (was tile columns). Task text says "env_id or owning instance"; a managed tile's volume is env-scoped (DECIDE 194); stack and org scopes are storage shares, so the volume carries the scope itself. Instance removal sets `instance_id` NULL and leaves the row (the flow sets `orphaned_at`, the schema never deletes data). `UNIQUE (scope_kind, scope_id, slug)`. Scope cascade by trigger. |
...
| `managed_instances` | managed | id, tile_id → tiles CASCADE, engine, allow (JSON list, default `[]`), env_pairs (JSON object, default `{}`), admin_user, admin_password (encrypted), endpoint, created_at | new (was `tiles.engine`/`db_*`). The instance container is its tile (`kind = managed`), env-scoped like any tile. `scope_kind`/`scope_id` gone (DECIDE 194): `allow` holds `org:stack:env:tile` patterns of slice tiles that may provision from it, `env_pairs` maps a consumer's env name to one of the instance stack's env names (the env segment of `provision_from`). Both are plain JSON, not encrypted. |
| `provisions` | managed | id, tile_id → tiles CASCADE UNIQUE, instance_id → managed_instances CASCADE, db_name, db_user, db_password (encrypted), public, on_remove, created_at | the slice itself, one per slice tile (DECIDE 194). `db_user`/`db_password` are the owner creds. Slice tile removal deletes it (and its bindings); `on_remove` says whether `flow/managed` drops the data first. `consumer_tile_id`, `slug` and `outputs` gone: consumers are `bindings`. `status` gone (derived). |
| `bindings` | managed | id, provision_id → provisions CASCADE, consumer_tile_id → tiles CASCADE, access `read\|write`, db_user, db_password (encrypted), outputs (encrypted JSON), created_at | new (DECIDE 194). One consumer's access to one slice, with its own role and creds; its outputs are what `${{ tile.<slice>.<output> }}` resolves to for that consumer. `UNIQUE (provision_id, consumer_tile_id)`. Consumer removal deletes it; the slice stays. |
