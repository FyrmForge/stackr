...
| table | leaf | columns | overrides vs 001 |
|---|---|---|---|
| `orgs` | org | id, name, slug UNIQUE, avatar_path, env_colors, settings, setup_done_at, created_at, config_connector_id, config_repo, config_branch, config_path, config_auto | `ui_edits`, `setup_mode` gone. `config_*` are the org config file's binding, the same four `stacks` has, plus `config_auto` (apply unblocked plans without a click, default off). `config_repo` is stored as the https URL. |
