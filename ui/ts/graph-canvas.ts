// <graph-canvas scale snap straight fan-out arrows focus divider>: pan,
// zoom (wheel, pinch, [data-zoom]) about the cursor, fit, shift+drag
// marquee, hover focus, "focus" centring, <svg data-edges> re-path. Writes
// --px/--py/--s on itself (CSS makes the transforms), "selected"/"lit" on
// nodes, "data-lit" on paths; fires "selection-changed" (detail.ids).
// Look attributes follow [data-look] checkboxes, kept in localStorage.
...
class GraphCanvas extends HTMLElement {
  ...
  // At least KEEP px of content stays on screen whichever way it pans.
  private apply(): void {
    const b = this.bounds();
    if (b) {
  ...
  // Empty canvas: drag pans (and drops the selection). Shift anywhere:
  // marquee. A second finger: pinch. Cards drag themselves.
  private onDown = (e: PointerEvent): void => {
    ...
    if (e.button !== 0 || this.pointers.size > 2 || t.closest("button, input, label, select, [data-zoom]")) return;
    if (e.shiftKey) {
  ...
  // Selects every card the rectangle touches (intersect, not contain).
  private endMarquee(select: boolean): void {
    const m = this.marquee;
    if (!m) return;
    this.marquee = null;
  ...
  private select(ids: Set<string>): void {
    ...
    if (had || ids.size > 0) this.dispatchEvent(new CustomEvent("selection-changed", { bubbles: true, detail: { ids: [...ids] } }));
  }
  // ctrl/meta (trackpad pinch) zooms smoothly, a mouse notch (lines, or a
  // whole ±100) zooms 1.1×, anything else is a two-finger scroll: pan.
  private onWheel = (e: WheelEvent): void => {
    e.preventDefault();
    if (e.ctrlKey || e.metaKey) return this.zoomAt(this.local(e), Math.exp(-e.deltaY * 0.01));
  ...
  private onOver = (e: PointerEvent): void => {
    if (this.pan || this.marquee || this.querySelector("graph-node[dragging]")) return;
    const n = e.type === "pointerleave" ? null : (e.target as Element).closest("graph-node");
    this.light(n && this.top(n));
  };
  // Lights a card, its edges and their far ends; CSS dims the rest.
  ...
  // order of their far ends. Cubic (control max(run/2, 60)) or straight.
  private repath(): void {
    const ends: End[] = [];
