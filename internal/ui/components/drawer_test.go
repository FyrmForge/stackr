...
func TestSideDrawerOnLoad(t *testing.T) {
	...
	if err := Layout(Shell{Drawer: templ.Raw("<p>tab body</p>"), Tab: "members"}, templ.NopComponent).Render(context.Background(), &b); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(b.String(), `<side-drawer open tab="members">`) || !strings.Contains(b.String(), `<div id="drawer-body"><p>tab body</p></div>`) {
