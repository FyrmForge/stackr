...
func TestEveryTabRenders(t *testing.T) {
	v := View{Node: "t1", Name: "api", Kind: "cron", Base: "/o/s/e/drawer/tile/api", Tab: "status", Error: "nope", Note: "deploy queued"}
	job := c.JobStatusView{Kind: "deploy", State: "running", StreamURL: "/o/s/e/drawer/jobs/j1/events", Live: true}
	for tab, tc := range map[string]struct {
		...
		"status": {Status(v, StatusView{Word: "running", Job: &job, Replicas: []ReplicaView{{Name: "api-1", State: "running"}}, NextRun: "Jul 25 03:00"}),
			[]string{`sse-connect="/o/s/e/drawer/jobs/j1/events"`, "api-1", "next Jul 25 03:00", `hx-post="/o/s/e/drawer/tile/api/restart"`, `word="api"`}},
		"logs": {Logs(v, LogsView{Replicas: []ReplicaView{{ID: "c1", Name: "api-1"}, {ID: "c2", Name: "api-2"}}, Container: "c2",
			Pane: c.LogPaneView{StreamURL: "/o/s/e/drawer/tile/api/logs/stream?container=c2"}}),
		...
		"backups": {Backups(v, BackupsView{Rows: []VolumeRow{{ID: "v1", Name: "uploads", Drawer: "/o/s/e/drawer/volume/v1?tab=backups"}}}), []string{"uploads", "drawer/volume/v1"}},
	} {
