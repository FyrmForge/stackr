...
func TestEveryTabRenders(t *testing.T) {
	...
	for tab, tc := range map[string]struct {
		...
		"env": {
			Env(v, EnvView{JSON: `{"A":"1"}`}),
			[]string{`name="env_json"`, `{&#34;A&#34;:&#34;1&#34;}`},
		},
		"settings": {
			Settings(v, c.SettingsFormView{
				ID:     "tile-settings",
				Action: "/x",
				Scope:  "Tile",
				Rows:   []c.SettingRowView{{Key: "cpu_limit", Type: "float"}},
			}),
			[]string{`id="tile-settings"`, "cpu_limit"},
		},
		"jobs": {
			Jobs(v, JobsView{
				Rows: []JobRow{
		...
		"image": {
			...
			[]string{"A new version is out.", "/image/check", `name="update_policy"`, `name="image_ref"`},
		},
		"runs": {
			Runs(v, RunsView{
				Cron: true,
		...
		for _, w := range append(
			...
			"nope",
			"deploy queued",
			`hx-push-url="?drawer=t1&amp;tab=runs"`,
		) {
