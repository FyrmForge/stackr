...
func TestSlicesRenders(t *testing.T) {
	...
	err := Slices(View{Name: "pg", Engine: "postgres", Scope: "stack", Slices: []SliceRow{
		{ID: "p1", Name: "shop", DB: "shop", Drawer: "/o/s/e/drawer/slice/p1?tab=bindings", Public: true},
		{ID: "p2", Name: "old", OnRemove: "drop", Orphan: true},
	}}).Render(context.Background(), &b)
	if err != nil {
		t.Fatal(err)
	}
	for _, w := range []string{"drawer/slice/p1", "public", "consumer gone, drop pending", "stack"} {
