...
func TestReleases(t *testing.T) {
	...
	for _, id := range []string{img1, img2} {
		must(t, st.Images.Create(ctx, store.Image{ID: id, Ref: "stkr/api:" + id, Digest: "sha256:" + id, CreatedAt: now}))
	}
	...
	r1, err := l.Create(ctx, stack, "push", []release.Pin{
		{Slug: "config", Repo: "acme/stack", Branch: "main", CommitSHA: "c1"},
		{Slug: "api", Repo: "acme/api", Branch: "main", CommitSHA: "a1", ImageID: &img1},
		{Slug: "db", Digest: "sha256:pg16"},
	})
	must(t, err)
	// A push to the api repo rebuilds api only; everything else is copied.
	r2, err := l.Derive(ctx, stack, r1.ID, "push", release.Pin{Slug: "api", Repo: "acme/api", Branch: "main", CommitSHA: "a2", ImageID: &img2})
	must(t, err)
	...
	if got := release.Diff(p1, p2); !reflect.DeepEqual(got, []release.Change{{"api", "changed"}, {"db", "removed"}, {"web", "added"}}) {
		t.Errorf("diff = %v", got)
	}
