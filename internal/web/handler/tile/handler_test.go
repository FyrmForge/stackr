...
func TestEveryTabServes(t *testing.T) {
	s := webtest.New(t)
	for _, tab := range []string{"status", "logs", "domains", "env", "settings", "jobs", "image", "backups", "nonsense"} {
...
func TestActions(t *testing.T) {
	...
	if rec.Code != 200 || !strings.Contains(rec.Body.String(), "GREETING") || !strings.Contains(rec.Body.String(), "saved") {
		t.Errorf("env = %d\n%s", rec.Code, rec.Body)
	}
	rec = s.Do(t, "POST", drawer+"/settings", url.Values{"cpu_limit": {"lots"}, "mem_limit_mb": {"256"}})
	if rec.Code != 422 || !strings.Contains(rec.Body.String(), "must be a number") || !strings.Contains(rec.Body.String(), `value="256"`) {
