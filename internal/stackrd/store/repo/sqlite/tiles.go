...
const tileCols = `id, stack_id, environment_id, name, slug, kind, source_type, git_url, git_branch, connector_id,
	...
	basic_auth_user, basic_auth_password, sec_headers, published_ports, traefik_override, watch_paths,
...
func (s *Store) UpdateTile(ctx context.Context, id string, cfg repo.TileConfig) error {
	...
	_, err := s.db.NamedExecContext(ctx,
		`UPDATE tiles SET name = :name, source_type = :source_type, git_url = :git_url,
		 ...
		 basic_auth_user = :basic_auth_user, basic_auth_password = :basic_auth_password, sec_headers = :sec_headers,
