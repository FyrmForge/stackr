...
// POST /logout
//
// The server-side delete has to succeed before the cookie is cleared. If it
// fails and we clear anyway, the browser forgets the token but the session row
// stays valid until it expires: the user is told they are logged out while
// anyone still holding that token value has a live session on their account.
// Staying logged in and saying so is the honest outcome, and leaves them able
// to retry.
func (h *handler) Logout(c echo.Context) error {
	ctx := c.Request().Context()
	cookie, err := c.Cookie(h.sessionManager.CookieName())
	if err == nil {
		session, _ := h.sessionManager.ValidateSession(ctx, cookie.Value)
		if session != nil {
			if err := h.sessionManager.DeleteSession(ctx, session.ID); err != nil {
				logging.FromContext(ctx).Error("logout: session not deleted, still signed in",
					"session", session.ID, "error", err)
				middleware.SetFlash(c, "Could not log you out. Try again.", middleware.FlashError)
				return respond.Redirect(c, "/")
			}
		}
	}
