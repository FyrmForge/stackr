// Package middleware holds this application's own Echo middleware, request
// logging with a propagated request id, the active-organization context, and
// the guard that keeps config-managed stacks read-only in the UI. Generic
// concerns (auth, CSRF, flash) come from hamr/pkg/middleware instead.
package middleware
